...
NetWatch 用于持续观察“这条连接现在到底稳不稳”。它既可以直接监测任意 `主机/IP + TCP 端口`，也可以通过代理客户端暴露的本地 SOCKS5 端口，测量“本机 → 代理节点 → Google”的完整 TLS 链路。

这里的“本地优先”是指桌面界面、配置、历史数据和监测 API 都留在本机，API 只监听 `127.0.0.1:9288`；探测任务本身当然会主动连接你配置的远端目标，节点模式还会连接 `www.google.com:443`。项目没有为每次探测启动 `ping.exe`、curl 或 Node.js 子进程，实际网络探测由轻量 Go sidecar 直接使用 TCP Socket 完成。
...
- 节点模式经过 `本地 SOCKS5 → 当前代理节点 → Google TLS`，不局限于 VLESS，也可用于 Hysteria2、Shadowsocks、Trojan 等任何能够提供本地 SOCKS5 入口的代理协议；
- 记录本地代理连接、SOCKS5 确认、TLS 首包、TLS 完成，以及可选的 Google HTTP 204；
...
flowchart LR
    ...
    SOCKS --> NODE["当前代理节点"]
    NODE --> GOOGLE["www.google.com:443<br/>TLS / 可选 HTTP 204"]
```
...
- Google HTTP 204：默认关闭，只在需要完整应用层回包验证时开启；
- 探测间隔：默认 5 秒，最低 2 秒；
- 总超时：默认 8 秒。

当前仅支持无需用户名和密码的 SOCKS5 入口。节点测试终点固定为 `www.google.com:443`，域名通过 SOCKS5 交给代理端解析，不使用本地 DNS。默认在证书校验和 TLS 握手完成后记为成功，不发送 HTTP 请求；开启“继续验证 Google HTTP 204”后，才会请求 `/generate_204` 并严格验证 204 响应。
...
一个普通本地 SOCKS5 端口通常只代表代理客户端当前选中的节点。切换节点后，已有目标的历史和动态延迟基准会混入前后两个节点；若希望统计完全隔离，应删除旧目标并新建目标，仅修改名称不会清空历史或基准。若要同时监测多个节点，需要为每个节点提供独立的本地 SOCKS5 端口或代理客户端实例。
...
| TLS 完成 | Google TLS 握手完成 | 节点模式的默认主延迟与尖峰判断依据 |
| Google 204 | 收到 HTTP 204 响应头 | 可选的完整应用层体验验证 |
...
- 节点模式的波动率始终使用 TLS 完成时间；P95 默认使用 TLS 完成时间，启用 HTTP 204 后则使用 Google 204 总延迟；
- 少于两个有效测量时，波动率显示为“—”；
...
- 丢包率是体验层估算，不是物理包级丢包率；
- 节点模式目前固定探测 Google TLS，不能自定义远端终点；
